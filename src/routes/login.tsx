...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Login — Skills021" },
      { name: "description", content: "Log in to your Skills021 account to access courses and your dashboard." },
    ],
  }),
  component: Login,
});
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl glass p-8">
        ...
        >
          ...
          <div className="text-right">
            <button type="button" onClick={() => toast("Reset link sent!")} className="text-xs text-primary">Forgot password?</button>
          </div>
          <button className="w-full rounded-full py-3 text-sm font-semibold btn-glow">Log In</button>
        </form>
